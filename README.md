# twedia

the video editor for your pocket!

Useful for clipping or removing audio/video, instantly and without any loss in quality;
[demo!](https://www.youtube.com/watch?v=Embpni9rP1s)


# Install

## Using Nix

This will compile the program automatically
```sh
nix profile install github:Elec3137/twedia
```

## Using flatpak

Download the `.flatpak` file from [releases](https://github.com/Elec3137/twedia/releases),
and install it with your app store of choice. (often just right click -> install)

Alternatively, here's a oneliner to fetch and install the latest release:
([source](https://gist.github.com/steinwaywhw/a4cd19cda655b8249d908261a62687f8?permalink_comment_id=5124113#gistcomment-5124113))
```sh
curl -L $(curl -s https://api.github.com/repos/Elec3137/twedia/releases/latest | grep '"browser_download_url":' | grep 'io.github.Elec3137.twedia.flatpak' |  grep -o 'https://[^"]*') -o /tmp/io.github.Elec3137.twedia.flatpak && flatpak install --user /tmp/io.github.Elec3137.twedia.flatpak
```


# Develop

The prefered way to develop is with `direnv` (`nix`) and `cargo`;
```sh
git clone https://github.com/Elec3137/twedia
cd twedia
direnv allow  # or `nix develop`
cargo build   # or `nix build`
cargo run     # or `nix run`
```


# TODO

## main goals

1. add feedback for when a preview is playing (eg red buttons)

    after trying to implement this, I realized this requires two different processes to call mutable methods on the `Child`:
    1. the process that waits for the `Child` to exit
    2. the process that might kill the `Child` due to a second press of the button

    I can think of three possible solutions:
    1. using another interface for controlling child processes
    2. storing the PID instead of working with the `Child` object
    3. mutable statics

2. consider looking into macOS support

3. consider looking into windows support

4. audio visualization (learn to work with ffmpeg audio data)

## style

1. consider using enum `Extreme` for differentiating `End` and `Start` for previews, to reduce code repetition

## flatpak

1. fix launching twedia with a file

2. use pipewire ([issue](https://github.com/flatpak/flatpak/issues/5130))

## cosmic

1. fix window decorations (should be able to drag it, close by clicking on the X)

2. fix button and text styles

## *meta*

ordering is by relevance/interest

~50% probablility any one thing will actually happen
